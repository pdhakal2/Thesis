...
h1genus<-read.csv(file="C:\\Users\\pdhakal\\Dropbox\\Thesis\\Aim2\\carcinoma sample\\genus\\genus_level_carcinoma_sample.csv", 
                  header=TRUE, stringsAsFactors = F, na.strings = "NA",row.names = 1)
rownames(h1genus) <- gsub(".*\\|g__","g__",rownames(h1genus))

h2genus <-read.csv(file="C:\\Users\\pdhakal\\Dropbox\\Thesis\\Aim2\\advanced adenoma sample\\genus\\genus_level_advance_adenoma_sample.csv", 
                   header=TRUE, stringsAsFactors = F, na.strings = "NA",row.names = 1)
rownames(h2genus) <- gsub(".*\\|g__","g__",rownames(h2genus))

h3genus <- read.csv(file="C:\\Users\\pdhakal\\Dropbox\\Thesis\\Aim2\\control sample\\genus\\genus_level_control_sample.csv", 
                    header=TRUE, stringsAsFactors = F, na.strings = "NA",row.names = 1)
rownames(h3genus) <- gsub(".*\\|g__","g__",rownames(h3genus))

#h <- cbind(h1genus,h2genus,h3genus)


abundance_table_carcinoma <- t(h1genus) #for healthy samples
abundance_table_adenoma <- t(h2genus) #for advance adenoma sample
abundance_table_healthy <- t(h3genus) #for carcinoma sample
...
for(boi in bacteria_list_carcinoma) {
  ...
  boi <- gsub("\\|",".",boi)
  #write.csv(file = paste("genus_level_regression/",boi,".regression_carcinoma.csv",sep=""),reg.result_carcinoma,row.names=F)
  if(nrow(reg.result.05_carcinoma)>0) write.csv(file = paste("Aim3/genus_level_regression/carcinoma/",boi,".regression.05_carcinoma.csv",sep=""),reg.result.05_carcinoma,row.names=F)
...
for(boi in bacteria_list_adenoma) {
  ...
  boi <- gsub("\\|",".",boi)
  #write.csv(file = paste("genus_level_regression/",boi,".regression_adenoma.csv",sep=""),reg.result_adenoma,row.names=F)
  if(nrow(reg.result.05_adenoma)>0) write.csv(file = paste("Aim3/genus_level_regression/adenoma/",boi,".regression.05_adenoma.csv",sep=""),reg.result.05_adenoma,row.names=F)
...
for(boi in bacteria_list_healthy[1]) {
  ...
  boi <- gsub("\\|",".",boi)
  #write.csv(file = paste("genus_level_regression/",boi,".regression_healthy.csv",sep=""),reg.result_healthy,row.names=F)
  if(nrow(reg.result.05_healthy)>0) write.csv(file = paste("Aim3/genus_level_regression/control/",boi,".regression.05_healthy.csv",sep=""),reg.result.05_healthy,row.names=F)
