...
aim3y<-read.csv(file="C:\\Users\\pdhakal\\Dropbox\\Thesis\\clinicalinfo.csv", header=TRUE, stringsAsFactors = F,na.strings = "n.a.")
...
h1species<-read.csv(file="C:\\Users\\pdhakal\\Dropbox\\Thesis\\Aim2\\carcinoma sample\\species\\species_level_carcinoma_sample.csv", 
                  header=TRUE, stringsAsFactors = F, na.strings = "NA",row.names = 1)
rownames(h1species) <- gsub(".*\\|s__","s__",rownames(h1species))

h2species <-read.csv(file="C:\\Users\\pdhakal\\Dropbox\\Thesis\\Aim2\\advanced adenoma sample\\species\\species_level_advance_adenoma_sample.csv", 
                   header=TRUE, stringsAsFactors = F, na.strings = "NA",row.names = 1)
rownames(h2species) <- gsub(".*\\|s__","s__",rownames(h2species))

h3species <- read.csv(file="C:\\Users\\pdhakal\\Dropbox\\Thesis\\Aim2\\control sample\\species\\species_level_control_sample.txt", 
                    header=TRUE, stringsAsFactors = F, na.strings = "NaN",row.names = 1,sep="\t")
#rownames(h3species) <- gsub(".*\\|s__","s__",rownames(h3species))

#h <- cbind(h1species,h2species,h3species)


abundance_table_carcinoma <- t(h1species) #for healthy samples
abundance_table_adenoma <- t(h2species) #for advance adenoma sample
abundance_table_healthy <- t(h3species) #for carcinoma sample
...
aim3y <- aim3y[,c(-15)]
#ggpairs(aim3y[,3:18])
for(i in 21:34) {print(levels(aim3y[,i]))}

#####regression carcinoma
#bacteria_list_carcinoma = c("k__Bacteria|p__Bacteroidetes|c__Bacteroidia|o__Bacteroidales|f__Bacteroidaceae|g__Bacteroides|s__Bacteroides_fragilis")
print( colnames(abundance_table_carcinoma))
#bacteria_list_carcinoma <-c("c__Clostridia","c__Actinobacteria","c__Bacteroidia")
...
for(i in 21:34) {print(levels(aim3y.carcinoma[,i]))}
...
for(boi in bacteria_list_carcinoma) {
  ...
  fit <- lm(bacteria ~ ., data = big.matrix_carcinoma[,1:24])
  ...
  boi <- gsub("\\|",".",boi)
  #write.csv(file = paste("species_level_regression/",boi,".regression_carcinoma.csv",sep=""),reg.result_carcinoma,row.names=F)
  if(nrow(reg.result.05_carcinoma)>0) write.csv(file = paste("Aim3/species_level_regression/carcinoma/",boi,".regression.05_carcinoma.csv",sep=""),reg.result.05_carcinoma,row.names=F)
...
for(boi in bacteria_list_adenoma) {
  ...
  boi <- gsub("\\|",".",boi)
  #write.csv(file = paste("species_level_regression/",boi,".regression_adenoma.csv",sep=""),reg.result_adenoma,row.names=F)
  if(nrow(reg.result.05_adenoma)>0) write.csv(file = paste("Aim3/species_level_regression/adenoma/",boi,".regression.05_adenoma.csv",sep=""),reg.result.05_adenoma,row.names=F)
...
for(boi in bacteria_list_healthy[1]) {
  ...
  boi <- gsub("\\|",".",boi)
  #write.csv(file = paste("species_level_regression/",boi,".regression_healthy.csv",sep=""),reg.result_healthy,row.names=F)
  if(nrow(reg.result.05_healthy)>0) write.csv(file = paste("Aim3/species_level_regression/control/",boi,".regression.05_healthy.csv",sep=""),reg.result.05_healthy,row.names=F)
